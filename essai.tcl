...
proc essai {nick host hand chan text} {
  if {[lsearch -exact [channel info $chan] "essai"] == -1} { return }
  ...
  } elseif {[regexp -nocase {((negitive|negative).(balance|money|credit)|overdrawn)} $text]} {
	...
	} elseif {[regexp -nocase {((signs?).{1,20}(don.?t|won.?t|aren.?t|does.not|are.not|are|stop(ped)?)?.{0,10}(disable|work|broke)|(enable|disable|turn.on|toggle|(can.?t).{1,10}(use|make|place|create)).{1,15}(signs?))} $text]} {
  ...
  } elseif {[regexp -nocase {(geoip)} $text]} {
  ...
  } elseif {[regexp -nocase {(((change|set|alter)(.?(to|the))?.?(lang|locale|english|german|french|dutch))|(lang|english|german|french|dutch|essentials).?(transe?lation|lang|locale))} $text]} {
  ...
  } elseif {[regexp -nocase {(help.*lain|lain.*help|lain:)} $text]} {
...
proc lainai {nick host hand chan text} {
  ...
  if {[info exists lainailist($nick)]} {
    if {[isvoice $nick $chan] || [isop $nick $chan] } {    
      unset lainailist($nick)
    } elseif {[regexp -nocase {\y(hello.?|help.?|question.?|anybody.?|nobody.?|hi|hey)\y} $text]} {
      unset lainailist($nick)
